...
// "condition" is double-quoted because it is a reserved word in MySQL (and the
// MySQL connection runs in ANSI_QUOTES mode so "" denotes an identifier there,
// as it natively does in SQLite and PostgreSQL).
...
func (s *Store) CreateAction(ctx context.Context, a *models.Action) error {
	...
	_, err = s.exec(ctx, `INSERT INTO actions (`+actionColumns+`)
		VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
...
func (s *Store) GetAction(ctx context.Context, uuid string) (*models.Action, error) {
	row := s.queryRow(ctx, `SELECT `+actionColumns+` FROM actions WHERE uuid = ?`, uuid)
...
func (s *Store) ListActions(ctx context.Context, tokenID string) ([]*models.Action, error) {
	rows, err := s.query(ctx, `SELECT `+actionColumns+`
...
func (s *Store) UpdateAction(ctx context.Context, a *models.Action) error {
	...
	res, err := s.exec(ctx, `UPDATE actions SET
...
func (s *Store) DeleteAction(ctx context.Context, uuid string) error {
	res, err := s.exec(ctx, `DELETE FROM actions WHERE uuid = ?`, uuid)
	if err != nil {
...
func (s *Store) nextActionPosition(ctx context.Context, tokenID string) int {
	var max *int
	_ = s.queryRow(ctx, `SELECT MAX(position) FROM actions WHERE token_id = ?`, tokenID).Scan(&max)
	if max == nil {
		return 1
	}
	return *max + 1
